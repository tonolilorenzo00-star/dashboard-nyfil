...
def page_elenco_clienti(df_clienti, df_ordini, anni_selezionati, paese_selezionato, analysis_mode):
    ...
    if not df_filtrato.empty:
        df_ranking_base = df_filtrato.groupby('CLIENTE').agg(Fatturato_Anagrafica=('FATTURATO', 'sum'), PAESE=('PAESE', 'first')).reset_index()
        ...
        if analysis_mode == "Aggrega Anni":
            if not df_ordini.empty and anni_selezionati:
                ...
                df_ranking = pd.merge(df_ranking_base, df_ordini_agg, left_on='CLIENTE', right_on='nome_cliente', how='left')
            ...
            for year in sorted(anni_selezionati, reverse=True):
                ...
                col_kg = f"KG_{year}"
                df_compare.rename(columns={year+'_x': col_fatt, year+'_y': col_kg}, inplace=True)
...
def page_analisi_dettagliata(df_clienti, df_ordini, anni_disponibili, anni_selezionati_globali, analysis_mode):
    ...
    # ... (Codice completo delle schede qui)
    with tab_eval:
